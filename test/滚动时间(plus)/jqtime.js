$(function() {
	...
	nowDate = new Date(nowDate.getTime()+1000*60*60*24*1);//是为了多一天
	
	var y = nowDate.getFullYear();
	var m = nowDate.getMonth()+1;
	var d = nowDate.getDate();
	var h = nowDate.getHours();
	console.log(y + "/"+m+ "/"+d+ " "+h + ":00");

	var arr = [];
	arr["year"] = String(y);
	arr["month"] = m<10 ? 0+String(m):String(m);
	arr["day"] = d<10 ? 0+String(d):String(d);
	arr["hour"] = h<10 ? 0+String(h):String(h);
	arr["minute"] = "00";
	$("#datetime").val(arr['year'] + "/" + arr['month'] + "/" + arr['day']+ " " + arr['hour']+ ":" + arr['minute']);


	// 初始高度
	$("#year").css({"transform": "translateY(" + (120-(y-2015+1)*30) + "px)"});
	$("#month").css({"transform": "translateY(" + (120-m*30) + "px)"});
	$("#day").css({"transform": "translateY(" + (120-d*30) + "px)"});
	$("#hour").css({"transform": "translateY(" + (120-(h+1)*30) + "px)"});
	$("#minute").css({"transform": "translateY(" + 90 + "px)"});

	timeMove($("#year"), true, 95, -65, "year");
	timeMove($("#month"), true, 95, -245, "month");
	timeMove($("#day"), true, 95, -815, "day");
	timeMove($("#hour"), true, 95, -605, "hour");
	timeMove($("#minute"), true, 95, 55, "minute");
